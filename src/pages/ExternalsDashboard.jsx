...
import { F, BRAND, NOISE_SVG, HamburgerIcon } from "../lib/constants.jsx";
...
export default function ExternalsDashboard({ session, onBack }) {
  ...
  const handleAdd = () => {
    if (activeNav === "events" && eventsRef.current) {
      eventsRef.current.add();
    } else if (activeNav === "opportunities" && opportunitiesRef.current) {
      opportunitiesRef.current.add();
    }
  };
  ...
  return (
    <div style={{
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", backgroundColor: BRAND.offWhite, backgroundImage: NOISE_SVG, minWidth: 0 }}>
        ...
        {activeNav === "opportunities"  && <Opportunities />}
      </main>

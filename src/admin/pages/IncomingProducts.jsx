// src/admin/pages/IncomingProducts.jsx
// Full CRUD admin dashboard for Global Volunteer, Global Talent, Global Teacher opportunities.
...
const PRODUCTS = [
  {
    key: 'volunteer', label: 'IG Volunteer', shortLabel: 'GV',
    color: '#e03131', lightBg: '#fff5f5', tabBg: '#fff0f0',
    icon: Heart,
    categories: ['Environment', 'Health', 'Education', 'Technology', 'Entrepreneurship'],
    durations: ['4 weeks', '6 weeks', '8 weeks'],
    specificField: { key: 'sdg_tag',  label: 'SDG Tag',      placeholder: 'e.g. SDG 13 – Climate Action' },
    badgeField:    { key: 'badge_text', label: 'Badge Text', placeholder: 'e.g. SDG 13' },
  },
  {
    key: 'talent', label: 'IG Talent', shortLabel: 'GT',
    color: C.blue, lightBg: C.blueLight, tabBg: '#eef6fe',
    icon: Briefcase,
    categories: ['Design', 'Marketing', 'Technology', 'Business', 'Human Resources'],
    durations: ['3 months', '4 months', '6 months'],
    specificField: { key: 'stipend',    label: 'Stipend Type', placeholder: 'Paid / Unpaid / Stipend' },
    badgeField:    { key: 'badge_text', label: 'Badge Text',   placeholder: 'e.g. Paid' },
  },
  {
    key: 'teacher', label: 'IG Teacher', shortLabel: 'GTe',
    color: '#d97706', lightBg: '#fffbeb', tabBg: '#fffaeb',
    icon: BookOpen,
    categories: ['English', 'STEM', 'Arts', 'Technology', 'Early Childhood', 'Physical Education'],
    durations: ['4 weeks', '6 weeks', '8 weeks'],
    specificField: { key: 'housing',    label: 'Housing',    placeholder: 'Provided / Self-arranged' },
    badgeField:    { key: 'badge_text', label: 'Badge Text', placeholder: 'e.g. 🏠 Housing Provided' },
  },
];
...
export default function IncomingProducts() {
  ...
  const handleSave = async (e) => {
    ...
    if (editRow) {
      ...
      ({ error } = await supabase.from('opportunities').insert(rest));
    }
    setSaving(false);
  ...
  const toggleVisibility = async (row) => {
    const { error } = await supabase.from('opportunities').update({ is_visible: !row.is_visible }).eq('id', row.id);
    if (!error) fetchOpps();
  };

  const toggleFeatured = async (row) => {
    const { error } = await supabase.from('opportunities').update({ is_featured: !row.is_featured }).eq('id', row.id);
    if (!error) fetchOpps();
  };
        ...
        <div style={{
          ...
          <div style={{ padding: '0 22px', display: 'flex', alignItems: 'center', gap: 12, borderRight: `1px solid rgba(255,255,255,0.1)`, flexShrink: 0 }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: F.rounded, fontWeight: 600, color: 'rgba(255,255,255,0.4)', fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.15em', marginTop: 3 }}>Incoming Opportunities</div>
            </div>
          </div>
